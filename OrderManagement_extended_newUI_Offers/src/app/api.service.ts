...
async purchaseCart(amt:number,cart:number): Promise<any>{
  try {
    const response = await this.http.put(`https://localhost:7044/purchaseCart?amt=${amt}&cartVal=${cart}`, null, {
      responseType: 'text',
...
async getVoucherAmount(): Promise<any>{
  try{
    const response=await this.http.get(`https://localhost:7044/getVoucherAmount`).toPromise();
    return response;
    }
    catch (error) {
      console.error('Error retrieving voucher amount:', error);
      throw error;
    }
}
}